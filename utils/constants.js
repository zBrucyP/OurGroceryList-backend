constants = {
    ERROR_MESSAGE_FAILED_LOGIN: 'Account was not found or password was incorrect.',
    ERROR_MESSAGE_SIGNUP_FAILED: 'Failed to signup user',
    ERROR_MESSAGE_ADD_LIST_FAILED: 'Could not add list',
    ERROR_SCHEMA_VALIDATION: 'ERROR_SCHEMA_VALIDATION', 
    ERROR_DB_CALL_FAILED: 'ERROR_DB_CALL_FAILED',
    ERROR_INFO_NOT_PROVIDED: 'Necessary info was not provided',
    ERROR_GET_LISTS_FAILED: 'Issue getting lists for user',
    ERROR_DELETE_LIST_FAILED: 'Could not delete list',
    ERROR_GET_LIST_ITEMS_FAILED: 'Could not get list items',
    ERROR_ADD_LIST_ITEMS_FAILED: 'Could not add list items',
    ERROR_UPDATE_LIST_ITEMS_FAILED: 'Could not update list items',

    TABLE_USERS: 'users',
    TABLE_LISTS: 'lists',
    TABLE_LIST_ITEMS: 'list_items',

    SUCCESS_MESSAGE_LIST_ADDED: 'List was successfully added',
}

module.exports = {
    constants,
}